...
Tests the entire simulation for a modulated PyWFS -- do we reconstruct the 
wavefront we put into the simulation?
...
def plot_phases(fname='phase_comparison.html'):
    ...
    for i in range(len(aberrs)):
        ...
        axs[1,i].axis('off')
    plt.tight_layout()
    plt.savefig('wavefront_reconstruction_modulated.png', dpi=300)
...
if __name__ == "__main__":
    ...
    modulation_radius = 0.15 # arcsecond
    ...
    for i, phase in enumerate(aberrs):
        ...
        hdu.writeto(f'./aberrations/recovered_modulated_{i}.fits', overwrite=True)
