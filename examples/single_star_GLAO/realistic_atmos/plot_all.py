...
def plot_errors(fname='performance.png'):
    files = ['./simulation1/simulation1.txt', 
             ...
             './simulation4/simulation4.txt']
    
    names = ['GLAO', 'ESM', '$PyArr_{masked}$', '$PyArr_{unmasked}$']
    linestyles = ['-', '--', ':', ':']
    colors = ['k', 'k', 'k', 'k']
    alphas = [1, 1, 1, 0.5]
    plate_scales = [3/500, 3/500, 3/500, 3*0.75/500]
    ...
    for i, file in enumerate(files):
        ...
        plt.plot(data['num_screens']*1e3, data['corrected_ee50']*plate_scales[i],
                 label=names[i], linestyle=linestyles[i], 
                 color=colors[i], alpha=alphas[i], linewidth=1)
    
    plt.plot(data['num_screens']*1e3, data['uncorrected_ee50']*plate_scales[i], 
             label='uncorrected', color='red', linewidth=1)
