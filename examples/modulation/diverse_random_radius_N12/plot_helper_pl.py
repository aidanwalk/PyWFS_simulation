...
def plot_response(data_file,
    ...
    # plt.axis('equal')
    plt.title(title)
    plt.xlabel('input rms wavefront error [radians]')
    plt.ylabel('output rms wavefront error [radians]')
    # plt.ylim(-0.06, 0.01)
    plt.savefig(fname, dpi=300)
    return
...
def make_gif(images, fname='animation.gif'):
    ...
    for i in range(len(images)):
        im = plt.imshow(images[i], animated=True, cmap='bone',)
        if i == 0:
